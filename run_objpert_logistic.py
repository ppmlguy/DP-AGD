...
from agd.common.logistic import logit_loss_and_grad
from sklearn.model_selection import RepeatedKFold
from agd.algo.objperturb import obj_pert


def main(args):
    fpath = "./dataset/{0}.dat".format(args.dname)
    X, y = load_dat(fpath, minmax=(0, 1), normalize=True, bias_term=True)
    N, dim = X.shape
    ...
    for train, test in rkf.split(X):
        ...
        for i, eps in enumerate(epsilon):
            for j in range(nrep):
                sol = obj_pert(train_X, train_y, eps, delta,
                               logit_loss_and_grad, 1.0, 1.0,
    ...
    filename = "objpert_logres_{0}".format(args.dname)
